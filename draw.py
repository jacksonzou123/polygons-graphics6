...
def draw_polygons( polygons, screen, color ):
    lines = []
    for i in range(len(polygons) // 3):
        a = [polygons[i * 3 + 1][0] - polygons[i * 3][0], polygons[i * 3 + 1][1] - polygons[i * 3][1], polygons[i * 3 + 1][2] - polygons[i * 3][2]]
        b = [polygons[i * 3 + 2][0] - polygons[i * 3][0], polygons[i * 3 + 2][1] - polygons[i * 3][1], polygons[i * 3 + 2][2] - polygons[i * 3][2]]
        n = [a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]]
        v = [0, 0, 1]
        if n[0] * v[0] + n[1] * v[1] + n[2] * v[2] > 0:
            add_edge(lines, polygons[i * 3][0], polygons[i * 3][1], polygons[i * 3][2], polygons[i * 3 + 1][0], polygons[i * 3 + 1][1], polygons[i * 3 + 1][2])
            add_edge(lines, polygons[i * 3 + 1][0], polygons[i * 3 + 1][1], polygons[i * 3 + 1][2], polygons[i * 3 + 2][0], polygons[i * 3 + 2][1], polygons[i * 3 + 2][2])
            add_edge(lines, polygons[i * 3 + 2][0], polygons[i * 3 + 2][1], polygons[i * 3 + 2][2], polygons[i * 3][0], polygons[i * 3][1], polygons[i * 3][2])
...
def add_sphere(polygons, cx, cy, cz, r, steps ):
    ...
    for i in range(len(points)):
        add_polygon(polygons, points[i][0], points[i][1], points[i][2], points[(i+1) % len(points)][0], points[(i+1) % len(points)][1], points[(i+1) % len(points)][2], points[(i+steps+1) % len(points)][0], points[(i+steps+1) % len(points)][1], points[(i+steps+1) % len(points)][2])
        if i % steps != 0:
            add_polygon(polygons, points[i][0], points[i][1], points[i][2], points[(i+steps+1) % len(points)][0], points[(i+steps+1) % len(points)][1], points[(i+steps+1) % len(points)][2], points[(i+steps) % len(points)][0], points[(i+steps) % len(points)][1], points[(i+steps) % len(points)][2])
